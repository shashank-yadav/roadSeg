...
float getWeight(const Vec3b &Ib1 , const Vec3b &Ib2 )
{
    ...
    // cout<<weight<<endl;
    // return 1000*weight/(1+abs(t1-t2));
    return 1e3*weight/(1+abs(t1-t2));
...
int main(int argc, char** argv)
{
    ...
    Mat roimap_hsv;
    cvtColor(roimap,roimap_hsv,CV_RGB2HSV);
...
// while(1)
{
    ...
    ifstream ifs("~/Documents/deepL/SegNet-Tutorial-master/kitti/data_road/training/data");
    ...
    while(len--)
    {
        ...
        // cout<<y<<" "<<x<<" "<<w1<<endl;
        // cout<<twoD_to_oneD(x,y,width)<<endl;
        g->add_tweights( twoD_to_oneD(y,x,width) , w1 , w2 );
    }
